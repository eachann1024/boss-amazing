...
export async function requestModelList(ai: AiConfig): Promise<ModelListResponse> {
  if (ai.provider === 'anthropic') {
    return requestClaudeModels(ai)
  }
  return requestOpenAICompatibleModels(ai)
}
...
async function requestOpenAICompatibleModels(ai: AiConfig): Promise<ModelListResponse> {
  const baseUrl = (ai.baseUrl || 'https://api.openai.com/v1').replace(/\/$/, '')
  ...
  if (!res.ok) {
    const text = await res.text()
    throw new Error(`模型列表请求失败: ${res.status} ${text}`)
  }

  const data = await res.json()
  const models = Array.isArray(data?.data)
    ? data.data
      .map((item: unknown) => (typeof item === 'object' && item && 'id' in item ? String((item as { id: unknown }).id) : ''))
      .filter(Boolean)
    : []
...
async function requestClaudeModels(ai: AiConfig): Promise<ModelListResponse> {
  ...
  if (!res.ok) {
    const text = await res.text()
    throw new Error(`Claude 模型列表请求失败: ${res.status} ${text}`)
  }
