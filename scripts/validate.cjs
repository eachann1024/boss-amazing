...
const extensionRoot = path.resolve(__dirname, '..', 'dist', 'dev')
...
const fixturePath = '/tmp/boss-awesome-fixture.html'
const fixtureHtml = `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>Boss Awesome Fixture</title>
    <style>
...
;(async () => {
  const context = await chromium.launchPersistentContext(`/tmp/boss-awesome-profile-${Date.now()}`, {
    headless: false,
  ...
  await page.waitForSelector('#boss-awesome-root', { timeout: 10000, state: 'attached' })

  const settingsBtn = page.getByTitle('Boss Awesome 设置')
  await settingsBtn.click({ timeout: 10000 })
  await page.waitForSelector('text=Boss Awesome · 设置')
